...
def build_test_plan() -> None:
    ...
    style_title(doc, "Test Plan — FrinStore (IEEE 829)")
    ...
    doc.save(OUT / "Group6_FrinStore_Doc1_TestPlan.docx")


def build_system_report() -> None:
    d = load("03_system_test_report.json")
    doc = Document()
    style_title(doc, "System Test Report — FrinStore")
    ...
    doc.save(OUT / "Group6_FrinStore_Doc5_SystemTestReport.docx")


def build_uat_report() -> None:
    d = load("04_uat_plan_and_report.json")
    doc = Document()
    style_title(doc, "UAT Plan & Report — FrinStore")
    ...
    doc.save(OUT / "Group6_FrinStore_Doc6_UATPlanReport.docx")


def build_regression_report() -> None:
    d = load("06_regression_test_report.json")
    doc = Document()
    style_title(doc, "Regression Test Report — FrinStore")
    doc.add_paragraph(f"Group: {d.get('group', '')} | Date: {d.get('report_date', '')}")
    ...
    doc.save(OUT / "Group6_FrinStore_Doc8_RegressionTestReport.docx")


def build_summary_report() -> None:
    d = load("07_test_summary_report.json")
    doc = Document()
    style_title(doc, "Test Summary Report — FrinStore")
    doc.add_paragraph(f"Group: {d.get('group', '')} | Date: {d.get('report_date', '')}")
    doc.add_paragraph(d.get("executive_summary", ""))
    ...
    doc.save(OUT / "Group6_FrinStore_Doc9_TestSummaryReport.docx")
...
def build_test_cases_xlsx() -> None:
    ...
    wb.save(OUT / "Group6_FrinStore_Doc2_TestCases.xlsx")
...
def build_defect_log_xlsx() -> None:
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        for c in row:
            c.alignment = Alignment(wrap_text=True, vertical="top")
    wb.save(OUT / "Group6_FrinStore_Doc7_DefectLog.xlsx")
